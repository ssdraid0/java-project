...
class TimSort<T>
{
    ...
    /**
     * 将要排序的数组 The array being sorted.
     */
    private final T[] a;
    ...
    /**
     * 按照归并排序算法，一个数组分为多块， runBase存储每个分块的首个元素在数组的位置。
     */
    ...
    static <T> void sort(T[] a, int lo, int hi, Comparator<? super T> c, T[] work, int workBase, int workLen)
    {
        ...
        // nRemaining一定大于等于32，minRun是16到32之间（包括边界）的一个数
        int minRun = minRunLength(nRemaining);
        do
        {
            // Identify next run
            // runLen位置之前（不包括runLen位置）的元素都是递增
            int runLen = countRunAndMakeAscending(a, lo, hi, c);

            // If run is short, extend to min(minRun, nRemaining)
            if (runLen < minRun)
            {
                // 实际nRemaining不可能小于minRun，force的值一定为minRun
    ...
    /**
     * 找到一个位置，这个位置之前（不包括这个位置）的元素都是递增(可以出现连续相同)或递减，如果是递减，这个位置之前（不包括这个位置）的所有元素反转，
     * 最后返回这个位置之前（不包括这个位置）的所有元素的长度。 Returns the length of the run beginning at
     * the specified position in the specified array and reverses the run if it
     * is descending (ensuring that the run will always be ascending when the
     * method returns).
     *
    ...
    /**
     * 如果n小于32，返回n，如果n大于等于32，n不断左移直到小于32，n是奇数还要加1。 Returns the minimum
     * acceptable run length for an array of the specified length. Natural runs
